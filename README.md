...
> 📋 실제 운영진을 위한 문서는 별도로 있습니다 — 일상 운영은 **[ADMIN_GUIDE.md](./ADMIN_GUIDE.md)**,
> 도메인 연결 전 확인 목록은 **[LAUNCH_CHECKLIST.md](./LAUNCH_CHECKLIST.md)**, 이번 프로덕션
> 재검수에서 실제로 무엇을 확인·수정했는지는 **[AUDIT.md](./AUDIT.md)**(PASS/PARTIAL/FAIL
> 매트릭스)를 참고하세요.
...
**전체 PASS/PARTIAL/FAIL/N/A 매트릭스는 [AUDIT.md](./AUDIT.md)를 참고하세요.** 아래는 요약만
남깁니다.

- `npm run build` (`tsc -b && vite build`) — 매 커밋마다 통과 확인. 빌드 산출물 약 647KB
  (gzip 197KB)이며, 코드 분할을 하지 않아 "chunk 500KB 초과" 경고가 표시되지만 빌드 자체는
  실패하지 않습니다.
- **FINAL PRODUCTION RE-AUDIT 세션에서 실제 브라우저 검증으로 발견하고 고친 항목** (자세한
  내용은 AUDIT.md): Home Hero 이미지 필드가 공개 화면에 전혀 반영되지 않던 버그, Story 커버
  이미지·About 섹션 이미지·Character 이미지가 관리자에서 입력해도 공개 화면에 나타나지 않던
  버그(4건 모두 admin에 입력 → 저장 → 공개 화면 확인 → 원복까지 라이브로 재검증), CoffeeCard와
  Stories 카드가 테두리 박스 + 아이콘뿐인 제네릭 그리드였던 문제(실제 fritz.co.kr,
  unspecialty.com을 방문해 UX 원리만 참고해 이미지 우선 에디토리얼 카드로 재설계), Brew Guide
  상세의 영어 섹션 라벨, robots.txt/sitemap.xml 부재, 관리자 대시보드의 지난 원두/정보 보완
  필요/최근 수정 누락.
- 그 밖에 Chrome 브라우저로 실제 실행해 확인한 흐름: 헤더 검색 → `/coffees?q=...` 자동 필터링,
  관리자 로그인 → 홈 관리 Hero/섹션 토글 → 공개 반영, 코이노커피/납품·교육 관리 화면, 문의 폼
  제출 → 관리자 반영, 커피 사전 신규 용어 생성 → 공개 반영, 원두 차트 목록/상세, 원두 편집기
  로스터 코멘트/Advanced Roast Data 입력 → 원두 차트 반영, 레거시 원두 데이터 마이그레이션.
- **여전히 실행하지 못한 것**: 자동화 E2E(Playwright)와 Lighthouse는 이 환경에 설치되어 있지
  않고 새로 설치할 네트워크 접근도 확인되지 않아 실행하지 못했습니다. **실제 390px 모바일
  스크린샷 QA도 이번에도 하지 못했습니다** — `resize_window` 도구가 `window.innerWidth`를
  실제로 바꾸지 못한다는 것을 8회 이상 재시도로 직접 확인한 환경 자체의 한계이며(AUDIT.md
  "환경 제약" 참고), 회피가 아니라 재현 가능한 도구 실패입니다. 대신 전 페이지에 Tailwind
  반응형 클래스(`sm:`/`lg:`/`xl:`)가 일관되게 적용되어 있는지 코드 레벨로 전수 검토했습니다.
  **배포 전 실제 휴대폰으로 반드시 재확인하세요** (LAUNCH_CHECKLIST.md에 포함).
...
- **SEO는 클라이언트 사이드**: Vite SPA이므로 `react-helmet-async`로 `<title>`/메타 태그를
  ...
  밖).
- **`sitemap.xml`/`robots.txt`는 정적 라우트만 포함**: `public/sitemap.xml`, `public/robots.txt`가
  생성되어 있지만, 원두/사전/스토리 상세 페이지처럼 LocalStorage에만 존재하는 동적 slug는
  빌드 시점에 읽을 백엔드가 없어 포함하지 못했습니다. 또한 `HashRouter`(`/#/...`) 특성상
  `robots.txt`의 `Disallow: /#/admin`은 대부분의 크롤러가 해시 뒷부분을 아예 서버에 보내지
  않기 때문에 실질적으로 강제되지 않습니다 — `/admin` 접근 차단은 비밀번호 게이트가 담당합니다.
  실제 도메인 배포 전 `sitemap.xml`/`robots.txt`의 `example.com`을 실제 도메인으로 교체해야
  합니다(LAUNCH_CHECKLIST.md 참고).
- **React Error Boundary 없음**: 컴포넌트 트리 어딘가에서 런타임 에러가 발생하면 커스텀 에러
  화면 대신 흰 화면이 됩니다. 다음 세션 작업 대상으로 이월합니다.
- **중앙 미디어 라이브러리·파일 업로드 없음**: 모든 이미지는 URL 입력 방식이며, 값이
  비어 있으면 자동으로 KOI 브랜드 타이포그래피 플레이스홀더(남색 배경 + 별 + 텍스트)가
  표시됩니다 — 빈 박스나 깨진 이미지 아이콘이 노출되지 않습니다.
- **Analytics 미연결**: GA4 등 분석 스크립트를 삽입할 지점은 없으며, 실제 Analytics 계정이
  결정된 뒤 `index.html`에 스크립트를 추가하는 작업이 필요합니다.
