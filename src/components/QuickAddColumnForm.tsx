...
export default function QuickAddColumnForm({ onClose, onCreated }: { onClose: () => void; onCreated: (column: Column) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy/40 px-4 py-8" onClick={onClose}>
      <div
        className="max-h-[90vh] w-full max-w-[560px] overflow-y-auto border border-line/15 bg-surface p-7"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] font-semibold tracking-[0.25em] text-accent font-kicker">QUICK WRITE</p>
            <h2 className="mt-1 text-[18px] font-bold text-ink">새 칼럼 작성</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
          <div>
            <span className="mb-1 block text-[10px] font-semibold tracking-[0.1em] text-ink/50">제목 *</span>
            <input
              value={form.title}
          ...
          <div>
            <span className="mb-1 block text-[10px] font-semibold tracking-[0.1em] text-ink/50">트렌드 요약 · 오늘의 소식 *</span>
            <textarea
          ...
          <div>
            <span className="mb-1 block text-[10px] font-semibold tracking-[0.1em] text-ink/50">코이노니아의 시각 *</span>
            <textarea
          ...
          <div>
            <span className="mb-1 block text-[10px] font-semibold tracking-[0.1em] text-ink/50">우리 매장 이야기 (선택)</span>
            <textarea
          ...
          <div>
            <span className="mb-1 block text-[10px] font-semibold tracking-[0.1em] text-ink/50">마무리 한마디 (선택)</span>
            <textarea
              value={form.closing}
          ...
          <div>
            <span className="mb-1 block text-[10px] font-semibold tracking-[0.1em] text-ink/50">참고 출처 (선택, 한 줄에 하나씩)</span>
            <textarea
              value={form.sources}
