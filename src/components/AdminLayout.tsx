...
export default function AdminLayout({ children }: { children: ReactNode }) {
  return (
    <div className="min-h-screen bg-canvas">
      <header className="border-b border-line/15 bg-surface">
        <div className="mx-auto flex max-w-[1240px] items-center justify-between px-6 py-4">
          <Link to="/admin" className="flex items-center gap-2">
            ...
            <span>
              ...
              <p className="mt-0.5 block font-serif text-[18px] font-bold tracking-tight text-ink">KOINONIA — 관리자</p>
            </span>
          </Link>
          <div className="flex items-center gap-2">
            <Link
              to="/"
              className="border border-line/25 px-3.5 py-2 text-[12px] font-semibold tracking-wide text-ink/70 hover:border-line hover:text-ink"
            >
              사이트 보기
            </Link>
            <button
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `px-3 py-1.5 text-[11px] font-semibold tracking-wide ${
                  isActive ? 'border border-line bg-navy text-warm-white' : 'border border-transparent text-ink/55 hover:text-ink'
                }`
              }
            >
              {item.label}
