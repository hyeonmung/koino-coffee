  ...
  return (
    <div className="flex min-h-screen flex-col bg-canvas">
      ...
      <main className="w-full min-w-0 lg:flex-1 mx-auto max-w-[720px] px-6 py-10">
        ...
        <h1 className="mt-1 text-[28px] font-bold leading-tight whitespace-pre-line text-ink">{post.title}</h1>
        <p className="mt-2 text-[11px] text-ink/40">{new Date(post.publishedDate).toLocaleDateString('ko-KR')}</p>
        ...
        {post.relatedLinks.length > 0 && (
          <div className="mt-8 flex flex-col gap-2 border-t border-line/15 pt-6">
            <p className="text-[10px] font-semibold tracking-[0.15em] text-ink/40">문의하기</p>
            {post.relatedLinks.map((link) => (
              <a
                ...
                className="inline-block w-fit border border-line px-4 py-2 text-[12px] font-semibold text-ink hover:bg-navy hover:text-warm-white"
              >
                {link.label} →
              </a>
            ))}
        ...
        {post.category === 'WHOLESALE' && (
          <section className="mt-10 border-t border-line/15 pt-8">
            ...
            <h2 className="mt-1 text-[20px] font-bold text-ink">원두 납품 신청</h2>
            <p className="mt-2 text-[12px] text-ink/55">아래 정보를 남겨주시면 확인 후 순차적으로 안내드리겠습니다.</p>
            <div className="mt-5">
              <WholesaleOrderForm />
            </div>
          </section>
