    ...
    <AdminLayout>
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 font-serif text-[24px] font-bold text-ink">홈 관리</h1>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={handleSave}
            className="border border-line bg-navy px-5 py-2.5 text-[12px] font-semibold text-warm-white hover:bg-navy-light"
          >
            저장
          </button>
        </div>
      ...
      <div className="mt-8 max-w-[680px] space-y-8">
        <section className="border border-line/15 bg-surface p-6">
          <h2 className="mb-4 font-serif text-[15px] font-bold text-ink">01 · Hero</h2>
        ...
        <section className="border border-line/15 bg-surface p-6">
          <h2 className="mb-2 font-serif text-[15px] font-bold text-ink">Featured Coffee</h2>
          <p className="text-[12px] text-ink/55">
            홈 화면 "지금 만날 수 있는 커피"에 노출할 원두는{' '}
            <Link to="/admin/coffees" className="font-semibold text-ink underline">
              원두 관리
            </Link>
        ...
        <section className="border border-line/15 bg-surface p-6">
          <h2 className="mb-4 font-serif text-[15px] font-bold text-ink">섹션 공개 / 숨김</h2>
          <div className="space-y-2">
            {SECTIONS.map((section) => (
              <label key={section.key} className="flex items-center gap-2 text-[12px] text-ink">
                <input
                  type="checkbox"
