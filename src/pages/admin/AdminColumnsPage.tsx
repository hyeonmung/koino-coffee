...
function statusBadge(column: Column) {
  if (column.publishStatus !== 'published') return { label: '비공개', cls: 'text-ink/40' }
    ...
    <AdminLayout>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="mt-1 font-serif text-[24px] font-bold text-ink">칼럼 관리</h1>
        </div>
        <div className="flex gap-2">
          ...
          <Link to="/admin/columns/new" className="border border-line/25 px-4 py-2.5 text-[12px] font-semibold text-ink/70 hover:border-line hover:text-ink">
            + 낱개로 쓰기
          </Link>
        </div>
      </div>
      ...
      <div className="mt-6 space-y-2">
        {columns.map((column) => {
          ...
          return (
            <div key={column.id} className="flex items-center justify-between border border-line/15 bg-surface px-4 py-3">
              <div>
        ...
        {columns.length === 0 && <p className="border border-line/15 bg-surface px-4 py-10 text-center text-[13px] text-ink/40">등록된 칼럼이 없습니다.</p>}
      </div>
    </AdminLayout>
  )
}
