...
export default function AdminColumnSchedulerPage() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const toSave: Column[] = activeRows.map((r) => {
        const base = r.date.slice(5).replace('-', '')
        let slug = base
        let n = 2
  ...
  if (done !== null) {
    return (
      <AdminLayout>
        <div className="max-w-[520px] border border-line/15 bg-surface px-6 py-14 text-center">
          <p className="text-[10px] font-semibold tracking-[0.25em] text-accent font-kicker">완료</p>
          <p className="mt-2 text-[16px] font-semibold text-ink">칼럼 {done}건이 예약되었습니다.</p>
          <p className="mt-1 text-[12px] text-ink/55">각 예약 시각이 지나면 자동으로 공개됩니다.</p>
          <button
            type="button"
            onClick={() => navigate('/admin/columns')}
            className="mt-5 border border-line bg-navy px-4 py-2 text-[12px] font-semibold text-warm-white hover:bg-navy-light"
          >
            칼럼 목록으로
          </button>
        </div>
  ...
  return (
    <AdminLayout>
      ...
      <div className="mt-1 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-serif text-[22px] font-bold text-ink">이번 주 칼럼 예약하기</h1>
          <p className="mt-1 text-[12px] text-ink/55">월~일 7일치를 한 번에 작성하면, 각 요일에 아침 6~8시 사이 랜덤 시각으로 예약됩니다.</p>
        </div>
        <label className="block">
          <span className="mb-1 block text-[10px] font-semibold tracking-[0.1em] text-ink/60">시작하는 월요일</span>
      ...
      {existingInWeek.length > 0 && !confirming && (
        <div className="mt-4 border border-accent/40 bg-accent/10 px-4 py-3 text-[12px] text-ink/70">
      ...
      {!confirming ? (
        <>
          ...
          <div className="mt-3 space-y-5">
            {rows.map((row, i) => (
              <div key={i} className={`border ${row.skip ? 'border-line/10 bg-line/[0.02]' : 'border-line/15 bg-surface'} p-5`}>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <span className="text-[13px] font-bold text-ink">
                      {DAY_LABELS[i]}요일 <span className="font-normal text-ink/40">· {dates[i]}</span>
                    </span>
                    {!row.skip && (
                      <span className="flex items-center gap-1.5 text-[11px] text-ink/50">
                        ...
                        <input
                          ...
                          className="border border-line/25 px-1.5 py-1 text-[12px] text-ink"
                        />
                        <button
                          type="button"
          ...
          <button
            type="button"
            onClick={goToConfirm}
            className="mt-6 border border-line bg-navy px-5 py-2.5 text-[12px] font-semibold tracking-wide text-warm-white hover:bg-navy-light"
          >
            예약 확인하기
          </button>
        </>
        ...
        <div className="mt-6">
          <p className="text-[13px] font-semibold text-ink">아래 내용으로 예약을 확정할까요?</p>
          <div className="mt-3 divide-y divide-line/10 border border-line/15 bg-surface">
            {activeRows.map((r) => (
          ...
          <div className="mt-5 flex gap-2">
            ...
            <button
              ...
              disabled={submitting}
              className="border border-line bg-navy px-5 py-2.5 text-[12px] font-semibold tracking-wide text-warm-white hover:bg-navy-light disabled:opacity-50"
            >
