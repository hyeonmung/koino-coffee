...
import type { BusinessSection, SiteSettings } from '../../data/schema'
...
export default function AdminBusinessPage() {
  ...
  const updateSection = (index: number, p: Partial<BusinessSection>) => {
    const next = settings.businessSections.map((s, i) => (i === index ? { ...s, ...p } : s))
    patch({ businessSections: next })
  }

  const addSection = () =>
    patch({ businessSections: [...settings.businessSections, { key: slugifyKey('new'), title: '', body: '' }] })
  const removeSection = (index: number) =>
    patch({ businessSections: settings.businessSections.filter((_, i) => i !== index) })
    ...
    <AdminLayout>
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 font-serif text-[24px] font-bold text-navy">납품 · 교육 관리</h1>
        </div>
      ...
      <div className="mt-8 max-w-[680px] space-y-6">
        ...
        <div className="space-y-4">
          {settings.businessSections.map((section, i) => (
            <div key={section.key} className="border border-navy/15 bg-white p-5">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-semibold text-navy/40">서비스 {i + 1}</span>
