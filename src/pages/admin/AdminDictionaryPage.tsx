    ...
    <AdminLayout>
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 font-serif text-[24px] font-bold text-ink">커피 사전 관리</h1>
        </div>
        <button
          type="button"
      ...
      {editing && (
        <div className="mt-6 max-w-[560px] space-y-3 border border-line/15 bg-surface p-6">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setEditing(null)} className="border border-line/25 px-4 py-2 text-[12px] text-ink/60 hover:border-line">
              취소
            </button>
          </div>
        </div>
      ...
      <div className="mt-8 divide-y divide-line/10 border-t border-line/15">
        {terms.map((t) => (
          <div key={t.id} className="flex items-center justify-between py-3">
            <div>
              <p className="text-[13px] font-semibold text-ink">
                {t.term} {t.termKo && <span className="font-normal text-ink/45">· {t.termKo}</span>}
              </p>
              <p className="text-[11px] text-ink/50">{t.shortDefinition}</p>
            </div>
            <div className="flex shrink-0 gap-1.5">
              <button type="button" onClick={() => setEditing(t)} className="border border-line/20 px-2.5 py-1.5 text-[11px] text-ink/60 hover:border-line hover:text-ink">
                수정
              </button>
              {confirmingId === t.id ? (
                ...
                <button type="button" onClick={() => setConfirmingId(t.id)} className="border border-line/20 px-2.5 py-1.5 text-[11px] text-ink/60 hover:border-red-400 hover:text-red-500">
                  삭제
                </button>
              )}
            </div>
        ...
        {terms.length === 0 && <p className="py-10 text-center text-[13px] text-ink/40">등록된 용어가 없습니다.</p>}
      </div>
