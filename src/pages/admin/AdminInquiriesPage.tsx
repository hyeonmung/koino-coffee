...
export default function AdminInquiriesPage() {
  ...
  return (
    <AdminLayout>
      ...
      <h1 className="mt-1 font-serif text-[24px] font-bold text-ink">문의 관리</h1>

      <div className="mt-6 space-y-3">
        {inquiries.map((inquiry) => (
          <div key={inquiry.id} className="border border-line/15 bg-surface p-5">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                <p className="text-[14px] font-semibold text-ink">
                  {inquiry.companyName} <span className="font-normal text-ink/50">· {inquiry.contactName}</span>
                </p>
                ...
                {inquiry.interestArea && (
                  <p className="mt-1 inline-block border border-accent/40 bg-accent/10 px-1.5 py-0.5 text-[10px] font-semibold text-ink/70">
                    {inquiry.interestArea}
                  </p>
                )}
              </div>
            ...
            {inquiry.expectedVolume && <p className="mt-2 text-[12px] text-ink/55">예상 사용량: {inquiry.expectedVolume}</p>}
            {inquiry.message && <p className="mt-2 whitespace-pre-wrap text-[12px] text-ink/70">{inquiry.message}</p>}
            <div className="mt-3 flex items-center justify-between">
              <p className="text-[10px] text-ink/35">{new Date(inquiry.createdAt).toLocaleString('ko-KR')}</p>
              {confirmingId === inquiry.id ? (
                ...
                <button type="button" onClick={() => setConfirmingId(inquiry.id)} className="text-[10px] text-ink/35 hover:text-red-500">
                  삭제
                </button>
              )}
            </div>
        ...
        {inquiries.length === 0 && <p className="border border-line/15 bg-surface px-4 py-10 text-center text-[13px] text-ink/40">접수된 문의가 없습니다.</p>}
      </div>
    </AdminLayout>
  )
}
